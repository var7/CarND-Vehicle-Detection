...
def interpret_netout(image, netout):
    ...
    for box in boxes:
        ...
        if max_prob > THRESHOLD:
            left  = int((box.x - box.w/2) * image.shape[1]) + xmin
            right  = int((box.x + box.w/2) * image.shape[1]) + xmin
            bot  = int((box.y - box.h/2) * image.shape[0]) + ymin
            top  = int((box.y + box.h/2) * image.shape[0]) + ymin
